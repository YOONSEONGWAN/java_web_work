...
public interface BookRepository extends JpaRepository<Book, Long>{
	...
	public List<Book> findAllByOrderByNameDesc();
	...
	public List<Book> findAllQueury(); // 메소드명은 자유 
	
	/*
	 * 	특정 DB 에서만 실행될 수 있는 원래의 Query 문도 실행 가능
	 * 	- naviveQuery=true 옵션을 따로 주면 됨
	 */
	@Query(value="SELECT id, name, author, publisher FROM BOOK_INFO ORDER BY id DESC", nativeQuery = true)
