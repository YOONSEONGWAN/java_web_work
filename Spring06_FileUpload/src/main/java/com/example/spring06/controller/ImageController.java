...
@Controller
public class ImageController { // 이 메소드가 ImageServlet 역할을 한다.
	 ...
	 * 	"upload/yyy.png"
	 * 	"upload/zzz.gif"
	 * 	패턴이 오면 실제 해당 이미지를 읽어서 실제 이미지 데이터를 응답하는 컨트롤러 메소드 만들기
	 */
	...
	public ResponseEntity<InputStreamResource> image(@PathVariable("saveFileName") String name) throws IOException{
		/*
		 * 	{saveFileName}이라는 경로 변수에 담긴 내용을 추출해서 String name 매개변수에 담는 기능을 수행하는
		 * 	@PathVariable("saveFileName") 경로변수 어노테이션. name 에 담기는 내용은 xxx yyy zzz 중 담기기 전까진 모른다.
		 */
