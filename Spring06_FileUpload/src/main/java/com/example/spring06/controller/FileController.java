...
public class FileController {
	 ...
	 * 	있는 클래스에 @PropertiesSource 어노테이션이 설정되어 있어야 한다.
	 * 	
	 * 	custom.properties 에 있는 파일 업로드
	 *  경로를 읽어와서 필드에 담는다.
	 */
	@Value("${file.location}")
	private String fileLocation; // 파일 저장 위치
	...
	public ResponseEntity<InputStreamResource> download(String orgFileName, String saveFileName, long fileSize) {
		/*
		 * 	파일을 다운로드 해주기 위해서는 원본파일명, 저장된 파일명, 파일의 실제 크기가 필요하다
		 * 	원래는 다운로드 해줄 파일의 PK 를 이용해 DB에서 읽어와야 하나
		 * 	여기서는 GET 방식 파라미터로 넘어오는 값 활용 
		 * 	응답은 파일데이터가 와야함 -> 어케함?
		 */
	...
	public String fileUpload2(Model model, FileDto dto) {// dto 로 받는 대신 파일의 타입은 멀티파트파일로 받아야함
		...
		String filePath=fileLocation + File.separator + saveFileName;
		try {
			// 업로드 된 파일을 저장할 파일객체 생성
			File saveFile=new File(filePath);
			// 원하는 곳으로 파일을 이동 시킨다. (원하는 곳에 파일 저장)
			myFile.transferTo(saveFile);
		}catch(Exception e) {
			e.printStackTrace();
		}
		
		// 원래는 DB 에 저장해야 하지만 테스트를 위해 view 페이지에 전달해서 출력
	...
	public String fileUpload(String title, MultipartFile myFile, Model model) {// 인풋타입 이름이 myFile 이기 때문에 맞게 작성해줌
